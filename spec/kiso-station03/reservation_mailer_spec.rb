...
RSpec.describe ReservationMailer do
  ...
  describe '#reminder' do
    let(:email) { ReservationMailer.with(reservation: reservation).reminder }
    it "subjectを検証する" do
      expect(email.subject).to eq "予約前日の確認メールです"
    end
    it "送信先を検証する" do
    ...
    context "本文を検証する" do
      ...
      it '座席の表示があること' do
        expect(email.body).to include(ApplicationController.helpers.sheet_number(reservation.sheet_id))
      end
    end
  end
end